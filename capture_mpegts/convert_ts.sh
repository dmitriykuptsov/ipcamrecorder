...
ffmpeg -f mpegts -i "$1.raw" -copyts -vcodec copy -c:a aac -ignore_unknown -f mpegts -metadata service_provider="strangebit.io" -metadata service_name="IPCAM stream" "$1.ts" > /dev/null 2>&1
rm "$1.raw"
